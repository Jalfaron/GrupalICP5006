...
```{r}
bd_sen_excel <- read_excel("C:/Users/berna/OneDrive/Escritorio/R_proyectos/Ramo MADD/1TrabajoGrupalR/GrupalICP5006/2021_11_Senadores_Datos_Eleccion.xlsx", skip = 4)
```
...
```{r}
colnames(bd_sen_excel) <- c("nro_region", "region", "circ_sen", 
...
```{r}
bd_sen_excel <- bd_sen_excel |> select(region, circ_sen, 
...
```{r}
bd_sen_excel <- bd_sen_excel |>  group_by(region, lista, partido, 
...
```{r}
senadores <- bd_sen_excel |> select(region, circ_sen, distrito, 
                                      lista, pacto, partido, 
