...
gg_resumen <- resumen_genero |> 
  filter(prop_electas_dip > 0.1) |> 
  pivot_longer(cols = -1)

gg_resumen <- gg_resumen |> 
  mutate(name = as_factor(name),
    name = fct_relevel(name, "prop_militantes_mujeres",
                            "prop_candidatas_dip", "prop_electas_dip",
                            "prop_candidatas_sen", "prop_electas_sen"))

gg_resumen |> 
  ggplot(aes(x = fct_reorder(partido, value), y = value, fill = name)) +
  geom_col() +
  geom_text(aes(label = 
                  if_else(value > 0.1, paste0((round(value, 2)*100),"%"), "")),
            hjust = -0.2, size = 3) +
  scale_y_continuous(labels = percent) +
  scale_fill_manual(values = c("#884EA0", "#5DADE2", "#52BE80", "#2471A3", "#196F3D")) +
